...
const seedDate = () => {
    ...
    musicData.push(new Song("Dr Dre", "The Chronic", "3:33", "The Chronic", 5, "Hip Hop", "Dr-Dre.jpg"))
    musicData.push(new Song("Snoop Dog", "Gin & Juice", "3:55", "Gin & Juice", 5, "Hip Hop", "Snoop-Dog.jpg"))
}
...
const loadData = (data) => {
    ...
    for(const song of data) {
        ...
        searchResultsTable.append(songRow);
    }

    toggleSearchResults(data && data.length > 0);
}

const toggleSearchResults = (isVisible) => {
    if(isVisible) {
        searchResultsContainer.classList.remove('hide-search-results');
        searchResultsContainer.classList.add('show-search-results');
    }
    else {
        searchResultsContainer.classList.remove('show-search-results');
        searchResultsContainer.classList.add('hide-search-results');
    }
} 
...
const removeTableRows = () => {
    while(searchResultsTable.firstChild) {
        searchResultsTable.removeChild(searchResultsTable.firstChild);
    }
}
...
const inititalize = () => {
    searchResultsTable = document.querySelector('#searchResults');
